...
function MeetOurTeam({teamMembers}){
    const memberCard = teamMembers.map((member)=>{
        return(
         <div className="m-4"key={member.name}>
            ...
            <h3 className="text-3xl font-semibold text-accent m-2">{member.name}</h3>
            <h2 className="text-lg text-gray">{member.position} 
             <span className='m-2'>
    ...
    return (
        <section className="bg-gray-500/5">
          <h1 className="text-5xl text-secondary py-8 font-semibold"> 
          Meet Our <span className="text-accent">Team</span></h1>
          <div className="grid md:grid-cols-3 gap-x-20 gap-y-8 px-10">
            {memberCard}
          </div>
        </section>
    )
